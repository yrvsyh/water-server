...
int SerialPort::getFd() { return m->tty_fd; }

int SerialPort::read(void *data, int length) { return ::read(m->tty_fd, data, length); }
...
void SerialPort::termiosOptions(termios &tios, const OpenOptions &options)
{
    ...
    tios.c_iflag |= (options.xon ? IXON : 0) | (options.xoff ? IXOFF : 0) |
